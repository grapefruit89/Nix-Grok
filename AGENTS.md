...
13. **Repo-Hygiene (Schichten):** (1) Kanonisch nur `/etc/nixos`. (2) `AGENTS.md` Traps — `protect-repo-traps.sh` (`chattr +i`). (3) `audit-outside-repo.sh` + pre-commit. (4) `my.core.repo-guard` nach Rebuild. (5) IDE-Workspace = `/etc/nixos`. (6) `stage-nixos/` nur Trap — kein Inhalt.
