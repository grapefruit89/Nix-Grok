...
9. **Vor** Änderungen in `modules/` oder `lib/`: **nixos-docs MCP** — `search_chunks` → `list_doc_links` → `search_nix`; evaluierte Werte: `eval_config`; Options-Fallback: `query_manix`. Shell: `/etc/nixos/scripts/nix-agent-query.sh`. Bei Fehlern zuerst `triage_error`.
10. **nixos-MCP** = nixpkgs live; **nixos-docs** = dieses Repo. Evaluierte Werte: `eval_config` / `nix-agent-query.sh config`.
